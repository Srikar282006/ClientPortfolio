...
const Navbar = () => {
  ...
  return (
    <>
      <div className={`navbar shadow-sm fixed top-0 left-0 right-0 z-50 ${navbarBg}`}>
        <div className="navbar-start">
          ...
          >
            <svg
              ...
              className={`h-6 w-6 ${dark ? 'hover:text-black' : ''}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        <div className="navbar-center hidden lg:flex">
          <ul className={`menu menu-horizontal px-1 ${navbarText}`}>
            <li className={`${dark ? 'hover:bg-gray-800' : ''}`}>
              <a href="/">Home</a>
            </li>
            <li className={`${dark ? 'hover:bg-gray-800' : ''}`}>
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed top-0 right-0 h-full w-64 shadow-lg transform transition-transform duration-300 z-50 ${sidebarBg} ${
          isOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <div
          className={`flex justify-between items-center p-4 border-b ${sidebarText}`}
        >
          <h2 className={`text-lg font-bold ${sidebarText}`}>Srikar Acharya</h2>
          <button onClick={() => setIsOpen(false)} className={sidebarText}>
            ✕
          </button>
        </div>
